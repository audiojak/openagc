...
/// Merge adjacent deltas of the same kind: fewer, larger strings.
pub(crate) fn coalesce(events: Vec<AgentEventInfo>) -> Vec<AgentEventInfo> {
    let mut out: Vec<AgentEventInfo> = Vec::with_capacity(events.len());
    for e in events {
        match (out.last_mut(), e) {
            (Some(AgentEventInfo::TextDelta { text }), AgentEventInfo::TextDelta { text: more }) => {
                text.push_str(&more)
            }
            (Some(AgentEventInfo::ThinkingDelta { text }), AgentEventInfo::ThinkingDelta { text: more }) => {
                text.push_str(&more)
            }
            (_, e) => out.push(e),
        }
...
async fn forward(mut rx: mpsc::UnboundedReceiver<(SessionId, AgentEvent)>, events: EventBus) {
    while let Some(first) = rx.recv().await {
        ...
        let mut sessions: Vec<(SessionId, Vec<AgentEventInfo>)> = Vec::new();
        for (sid, e) in batch {
            match sessions.iter_mut().find(|(s, _)| *s == sid) {
                Some((_, list)) => list.push(e.into()),
                None => sessions.push((sid, vec![e.into()])),
            }
        }
        for (sid, list) in sessions {
            events.emit(CoreEvent::AgentEvents { session_id: sid.0, events: coalesce(list) });
        }
    }
}
...
impl Core {
    ...
    pub(crate) fn agent_runtime(&self) -> &AgentRuntime {
        self.agents.runtime.get_or_init(|| {
            ...
            runtime::runtime().spawn(forward(rx, self.events.clone()));
            AgentRuntime { manager: AgentManager::new(self.agent_providers(), "agent", tx.clone()), tx }
        })
    }
}
...
impl Core {
    ...
    pub async fn start_agent_session(
        ...
        self.db()?; // an account must be open: the tools read it
        ...
        let id = rt.manager.new_session_id();
